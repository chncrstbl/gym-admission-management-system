...
const EditEquipmentModal = ({ isOpen, onClose, onSubmit, isSaving, initialData }) => {
    const [formData, setFormData] = useState({
        name: '', category: 'Cardio', condition: 'Excellent', value: '', image: ''
    });
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                condition: initialData.equipment_condition || 'Excellent',
                value: initialData.asset_value || '',
    ...
    return (
        <BaseModal isOpen={isOpen} onClose={onClose} title="Edit Equipment Details">
            <form onSubmit={handleSubmit} className="space-y-4">
                
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 flex justify-between items-center mb-4">
                    <span className="text-xs font-bold text-gray-500 uppercase">Serial Code</span>
                    <span className="font-mono font-bold text-blue-700">{initialData.serial_code}</span>
                </div>
                ...
                <div>
                    ...
                    <input className="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" step="0.01" className="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={formData.value} onChange={e => setFormData({...formData, value: e.target.value})} />
                    </div>
                </div>

                <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Condition</label>
                    <select className="w-full border border-gray-300 p-2.5 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 outline-none" value={formData.condition} onChange={e => setFormData({...formData, condition: e.target.value})}>
                        {['Excellent', 'Good', 'Fair', 'Poor', 'Damaged'].map(cond => <option key={cond}>{cond}</option>)}
                    </select>
                </div>
